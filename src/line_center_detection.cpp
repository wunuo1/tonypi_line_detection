...
#include "line_center_detection.h"
...
LineCenterDetectionNode::LineCenterDetectionNode(const std::string& node_name,
  ...
  if (Init() != 0) {
    RCLCPP_ERROR(rclcpp::get_logger("LineCenterDetectionNode"), "Init failed!");
  }
...
int LineCenterDetectionNode::SetNodePara() {
  if (!dnn_node_para_ptr_) {
    return -1;
  }
  RCLCPP_INFO(rclcpp::get_logger("LineCenterDetectionNode"), "path:%s\n", model_path_.c_str());
...
void LineCenterDetectionNode::subscription_callback(
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("LineCenterDetectionNode"), "%s", ss.str().c_str());
  ...
  if (0 != ret) {
    RCLCPP_ERROR(rclcpp::get_logger("LineCenterDetectionNode"), "hbDNNWaitTaskDone failed!");
    hbSysFreeMem(&(input_tensor.sysMem[0]));
    hbSysFreeMem(&(output_tensor.sysMem[0]));
  }
  hbDNNReleaseTask(task_handle);
  if (0 != ret) {
    RCLCPP_ERROR(rclcpp::get_logger("LineCenterDetectionNode"), "release task failed!");
    hbSysFreeMem(&(input_tensor.sysMem[0]));
    hbSysFreeMem(&(output_tensor.sysMem[0]));
  }
  ...
  pyramid = hobot::dnn_node::ImageProc::GetNV12PyramidFromNV12Img(
      ...
      224);
  if (!pyramid) {
    RCLCPP_ERROR(rclcpp::get_logger("LineCenterDetectionNode"), "Get Nv12 pym fail!");
    return;
  }
  ...
  if (ret != 0) {
    RCLCPP_ERROR(rclcpp::get_logger("LineCenterDetectionNode"),
...
int LineCenterDetectionNode::Predict(
  std::vector<std::shared_ptr<DNNInput>> &dnn_inputs,
  const std::shared_ptr<DnnNodeOutput> &output,
  const std::shared_ptr<std::vector<hbDNNRoi>> rois) {
  RCLCPP_INFO(rclcpp::get_logger("LineCenterDetectionNode"), "input size:%d roi size:%d", dnn_inputs.size(), rois->size());
...
int32_t LineCoordinateParser::Parse(
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("LineCenterDetectionNode"),
...
int main(int argc, char* argv[]) {
  ...
  rclcpp::spin(std::make_shared<LineCenterDetectionNode>("LineCenterDetectionNode"));

  rclcpp::shutdown();

  RCLCPP_WARN(rclcpp::get_logger("LineCenterDetectionNode"), "Pkg exit.");
  return 0;
}
